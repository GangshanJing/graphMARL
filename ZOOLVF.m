...
N=9; gamma=0.8; T=10; eta=.01; r=2; K=600;
...
G(3,4)=1;
G(4,3)=1;
G(5,3)=1; G(5,6)=1; G(5,7)=1;
G(6,5)=1;
G(7,8)=1; G(8,9)=1; G(9,7)=1;
...
Gcm=(G+G')/2+eye(N); % The communication graph 
Gcm(Gcm~=0)=1;


n=4; % number of clusters
    
    LVF(1).index=[1 2 3 4];  % I_l^{cl}
    LVF(2).index=[3 4];
    LVF(3).index=[3 4 5 6 7 8 9];
    LVF(4).index=[7 8 9];
    LVF(1).V=[1 2];
    LVF(2).V=[3 4];
    LVF(3).V=[5 6];
...
for l=1:n
    GG=zeros(N);
    GG(LVF(l).index,LVF(l).index)=Gcm(LVF(l).index,LVF(l).index);
...
[J,~]=Evaluatetheta(theta,gamma,m00,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind,matrix,1,LVF); % 1 means global, 0 means local
...
ylabel('W(\theta^k,\xi^*)', 'fontsize',20)
set(gca,'fontsize',20)
%  axis([0 K -3 -1])   
