...
N=100; gamma=0.8; T=10; eta=.05; r=2; K=150;
...
[J,~]=Evaluatetheta(theta,gamma,m00,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind);
...
[Jc,~]=Evaluatetheta(thetag2,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the current global policy theta

[~,Vald]=Evaluatetheta(thetal2,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the current local policy theta
...
[Jgp,~]=Evaluatetheta(thetagp,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the perturbed policy thetap, obtain global value J    
   
[~,Vallp]=Evaluatetheta(thetalp,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the perturbed policy thetap, obtain local value J_i 
 
[Jg2p,~]=Evaluatetheta(thetag2p,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the perturbed policy thetap, obtain global value J    
   
[~,Vall2p]=Evaluatetheta(thetal2p,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the perturbed policy thetap, obtain local value J_i 
...
Jhg(k+1,h)=Evaluatetheta(thetag,gamma,m00,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the updated policy Thetag
Jhl(k+1,h)=Evaluatetheta(thetal,gamma,m00,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the updated policy Thetal 

Jhg2(k+1,h)=Evaluatetheta(thetag2,gamma,m00,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the updated policy Thetag
Jhl2(k+1,h)=Evaluatetheta(thetal2,gamma,m00,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind); % Evaluate the updated policy Thetal 
...
function [Jp,Value]=Evaluatetheta(theta,gamma,m0,N,A,sigma,phi,T,G,Gl,feature,Proj,lay,ind)
m=m0;
a=zeros(N);
d=zeros(N,1);
...
for i=1:N
    for j=1:N
        if Gl(j,i)==1  % including i itself
            Value(i)=Value(i)+LocaVal(j);
        end
    end
end
